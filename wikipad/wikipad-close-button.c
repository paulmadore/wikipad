...
#if ! GTK_CHECK_VERSION(3, 0, 0)
static void
wikipad_close_button_style_set (GtkWidget *widget,
...
wikipad_close_button_class_init (MousepadCloseButtonClass *klass)
{
  ...
  gtk_rc_parse_string (
    "style \"wikipad-close-button-style\" {\n"
    "  GtkWidget::focus-padding = 0\n"
    "  GtkWidget::focus-line-width = 0\n"
    "  xthickness = 0\n"
    "  ythickness = 0\n"
    "}\n"
    "widget \"*.wikipad-close-button\" style \"wikipad-close-button-style\"\n");

  widget_class = GTK_WIDGET_CLASS (klass);
  widget_class->style_set = wikipad_close_button_style_set;
#endif
}
...
wikipad_close_button_init (MousepadCloseButton *button)
{
  ...
  gtk_style_context_add_provider (context,
                                  GTK_STYLE_PROVIDER (WIKIPAD_CLOSE_BUTTON_GET_CLASS(button)->css_provider),
                                  GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
#else
  gtk_widget_set_name (GTK_WIDGET (button), "wikipad-close-button");
#endif
...
GtkWidget *
wikipad_close_button_new (void)
{
  return g_object_new (WIKIPAD_TYPE_CLOSE_BUTTON, NULL);
}

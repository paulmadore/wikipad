...
static gboolean
wikipad_util_iter_word_characters (const GtkTextIter *iter)
{
  gunichar c;
...
gboolean
wikipad_util_iter_starts_word (const GtkTextIter *iter)
{
  GtkTextIter prev;
  ...
  if (wikipad_util_iter_word_characters (&prev))
    return FALSE;
...
gboolean
wikipad_util_iter_ends_word (const GtkTextIter *iter)
{
  if (!gtk_text_iter_ends_word (iter))
    return FALSE;

  /* check if it's a character we'd like to see in a word */
  if (wikipad_util_iter_word_characters (iter))
    return FALSE;
...
gboolean
wikipad_util_iter_inside_word (const GtkTextIter *iter)
{
  GtkTextIter prev;

  /* not inside a word when at beginning or end of a word */
  if (wikipad_util_iter_starts_word (iter) || wikipad_util_iter_ends_word (iter))
    return FALSE;
  ...
  return wikipad_util_iter_word_characters (&prev);
}



gboolean
wikipad_util_iter_forward_word_end (GtkTextIter *iter)
{
  if (wikipad_util_iter_ends_word (iter))
    return TRUE;
  ...
  return wikipad_util_iter_ends_word (iter);
}



gboolean
wikipad_util_iter_backward_word_start (GtkTextIter *iter)
{
  /* return true if the iter already starts a word */
  if (wikipad_util_iter_starts_word (iter))
    return TRUE;
  ...
  return wikipad_util_iter_starts_word (iter);
}



gboolean
wikipad_util_iter_forward_text_start (GtkTextIter *iter)
{
  g_return_val_if_fail (!wikipad_util_iter_inside_word (iter), FALSE);
...
gboolean
wikipad_util_iter_backward_text_start (GtkTextIter *iter)
{
...
gchar *
wikipad_util_config_name (const gchar *name)
{
  const gchar *s;
  gchar       *config, *t;
  gboolean     upper = TRUE;
...
gchar *
wikipad_util_key_name (const gchar *name)
{
  const gchar *s;
  gchar       *key, *t;
...
gchar *
wikipad_util_utf8_strcapital (const gchar *str)
{
  gunichar     c;
  const gchar *p;
  gchar       *buf;
...
gchar *
wikipad_util_utf8_stropposite (const gchar *str)
{
  gunichar     c;
  const gchar *p;
  gchar       *buf;
...
gchar *
wikipad_util_escape_underscores (const gchar *str)
{
  GString     *result;
  const gchar *s;
...
GtkWidget *
wikipad_util_image_button (const gchar *stock_id,
                            const gchar *label)
{
...
void
wikipad_util_entry_error (GtkWidget *widget,
                           gboolean   error)
{
  ...
  pointer = wikipad_object_get_data (G_OBJECT (widget), "error-state");
  ...
  if (GPOINTER_TO_INT (pointer) != error)
    {
      ...
      /* set the new state */
      wikipad_object_set_data (G_OBJECT (widget), "error-state", GINT_TO_POINTER (error));
    }
}
...
void
wikipad_util_dialog_header (GtkDialog   *dialog,
                             const gchar *title,
...
gint
wikipad_util_get_real_line_offset (const GtkTextIter *iter,
                                    gint               tab_size)
{
  gint        offset = 0;
...
gboolean
wikipad_util_forward_iter_to_text (GtkTextIter       *iter,
...
gchar *
wikipad_util_get_save_location (const gchar *relpath,
...
void
wikipad_util_save_key_file (GKeyFile    *keyfile,
...
GType
wikipad_util_search_flags_get_type (void)
{
                           ...
                           const GtkTextIter   *limit)
{
  ...
  /* search properties */
  match_case       = (flags & WIKIPAD_SEARCH_FLAGS_MATCH_CASE) != 0;
  search_backwards = (flags & WIKIPAD_SEARCH_FLAGS_DIR_BACKWARD) != 0;
  whole_word       = (flags & WIKIPAD_SEARCH_FLAGS_WHOLE_WORD) != 0;
              ...
              if (G_LIKELY (!search_backwards))
                {
                  ...
                  if (whole_word && !(wikipad_util_iter_starts_word (&begin) && wikipad_util_iter_ends_word (&iter)))
              ...
              else
                {
                  ...
                  /* check if we match a whole word */
                  if (whole_word && !(wikipad_util_iter_starts_word (&iter) && wikipad_util_iter_ends_word (&begin)))
                    goto reset_and_continue_searching;
                }
                                ...
                                GtkTextIter         *iter)
{
  ...
  if (flags & WIKIPAD_SEARCH_FLAGS_AREA_DOCUMENT)
    {
      /* get document bounds */
      gtk_text_buffer_get_bounds (buffer, start, end);

      /* set the start iter */
      if (flags & WIKIPAD_SEARCH_FLAGS_ITER_AREA_START)
        *iter = *start;
      else if (flags & WIKIPAD_SEARCH_FLAGS_ITER_AREA_END)
        *iter = *end;
      else
        goto set_selection_iter;
    }
  else if (flags & WIKIPAD_SEARCH_FLAGS_AREA_SELECTION)
    {
      /* set area iters */
      *start = sel_start;
      *end = sel_end;
      ...
      /* set the start iter */
      if (flags & (WIKIPAD_SEARCH_FLAGS_ITER_AREA_START | WIKIPAD_SEARCH_FLAGS_ITER_SEL_START))
        *iter = sel_start;
      else if (flags & (WIKIPAD_SEARCH_FLAGS_ITER_AREA_END | WIKIPAD_SEARCH_FLAGS_ITER_SEL_END))
        *iter = sel_end;
      else
                         ...
                         MousepadSearchFlags  flags)
{
  ...
  g_return_val_if_fail ((flags & WIKIPAD_SEARCH_FLAGS_DIR_BACKWARD) == 0, -1);
  ...
  if (string == NULL || *string == '\0' || flags & WIKIPAD_SEARCH_FLAGS_ACTION_CLEANUP)
    return 0;
...
GtkAction *
wikipad_util_find_related_action (GtkWidget *widget)
{
  GtkAction *action;
...
GIcon *
wikipad_util_icon_for_mime_type (const gchar *mime_type)
{
  gchar *content_type;
  GIcon *icon = NULL;
...
static void
wikipad_util_container_foreach_counter (GtkWidget *widget,
                                         guint     *n_children)
{
...
gboolean
wikipad_util_container_has_children (GtkContainer *container)
{
  guint n_children = 0;
  ...
  gtk_container_foreach (container,
                         (GtkCallback) wikipad_util_container_foreach_counter,
                         &n_children);
...
void
wikipad_util_container_clear (GtkContainer *container)
{
  GList *list, *iter;
...
void
wikipad_util_container_move_children (GtkContainer *source,

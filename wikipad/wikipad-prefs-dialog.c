...
#include <wikipad/wikipad-prefs-dialog-ui.h>
...
struct WikipadPrefsDialogClass_
{
...
static void wikipad_prefs_dialog_finalize (GObject *object);



G_DEFINE_TYPE (WikipadPrefsDialog, wikipad_prefs_dialog, GTK_TYPE_DIALOG)



static void
wikipad_prefs_dialog_class_init (WikipadPrefsDialogClass *klass)
{
  GObjectClass *g_object_class;

  g_object_class = G_OBJECT_CLASS (klass);

  g_object_class->finalize = wikipad_prefs_dialog_finalize;
}



static void
wikipad_prefs_dialog_finalize (GObject *object)
{
  WikipadPrefsDialog *self;

  g_return_if_fail (WIKIPAD_IS_PREFS_DIALOG (object));

  self = WIKIPAD_PREFS_DIALOG (object);

  /* destroy the GtkBuilder instance */
  if (G_IS_OBJECT (self->builder))
    g_object_unref (self->builder);

  G_OBJECT_CLASS (wikipad_prefs_dialog_parent_class)->finalize (object);
}
...
static void
wikipad_prefs_dialog_color_scheme_changed (WikipadPrefsDialog *self,
                                            GtkComboBox         *combo)
{
  GtkListStore *store;
  GtkTreeIter   iter;
  ...
  self->blocked = TRUE;
  WIKIPAD_SETTING_SET_STRING (COLOR_SCHEME, scheme_id);
  self->blocked = FALSE;
...
static void
wikipad_prefs_dialog_color_scheme_setting_changed (WikipadPrefsDialog *self,
                                                    gchar               *key,
                                                    GSettings           *settings)
{
  gchar        *new_id;
  ...
  new_id = WIKIPAD_SETTING_GET_STRING (COLOR_SCHEME);
...
static void
wikipad_prefs_dialog_setup_color_schemes_combo (WikipadPrefsDialog *self)
{
  ...
  wikipad_prefs_dialog_color_scheme_setting_changed (self, NULL, NULL);

  g_signal_connect_swapped (gtk_builder_get_object (self->builder, WID_SCHEME_COMBO),
                            "changed",
                            G_CALLBACK (wikipad_prefs_dialog_color_scheme_changed),
                            self);

  WIKIPAD_SETTING_CONNECT_OBJECT (COLOR_SCHEME,
                                   G_CALLBACK (wikipad_prefs_dialog_color_scheme_setting_changed),
                                   self,
                                   G_CONNECT_SWAPPED);
}
...
static void
wikipad_prefs_dialog_tab_mode_changed (WikipadPrefsDialog *self,
                                        GtkComboBox         *combo)
{
  self->blocked = TRUE;
  /* in the combo box, id 0 is tabs, and 1 is spaces */
  WIKIPAD_SETTING_SET_BOOLEAN (INSERT_SPACES,
...
static void
wikipad_prefs_dialog_tab_mode_setting_changed (WikipadPrefsDialog *self,
                                                gchar               *key,
                                                GSettings           *settings)
{
  ...
  insert_spaces = WIKIPAD_SETTING_GET_BOOLEAN (INSERT_SPACES);
...
static void
wikipad_prefs_dialog_home_end_changed (WikipadPrefsDialog *self,
                                        GtkComboBox         *combo)
{
  self->blocked = TRUE;
  WIKIPAD_SETTING_SET_ENUM (SMART_HOME_END, gtk_combo_box_get_active (combo));
  self->blocked = FALSE;
}
...
static void
wikipad_prefs_dialog_home_end_setting_changed (WikipadPrefsDialog *self,
                                                gchar               *key,
                                                GSettings           *settings)
{
  GtkComboBox *combo;
  ...
  gtk_combo_box_set_active (combo, WIKIPAD_SETTING_GET_ENUM (SMART_HOME_END));
}
...
static void
wikipad_prefs_dialog_toolbar_style_changed (WikipadPrefsDialog *self,
                                             GtkComboBox         *combo)
{
  self->blocked = TRUE;
  WIKIPAD_SETTING_SET_ENUM (TOOLBAR_STYLE, gtk_combo_box_get_active (combo));
  self->blocked = FALSE;
}
...
/* update the combo when the setting changes */
static void
wikipad_prefs_dialog_toolbar_style_setting_changed (WikipadPrefsDialog *self,
                                                     gchar               *key,
                                                     GSettings           *settings)
{
  GtkComboBox *combo;
  ...
  gtk_combo_box_set_active (combo, WIKIPAD_SETTING_GET_ENUM (TOOLBAR_STYLE));
}
...
static void
wikipad_prefs_dialog_toolbar_icon_size_changed (WikipadPrefsDialog *self,
                                                 GtkComboBox         *combo)
{
  GtkTreeIter iter;
  ...
  if (gtk_combo_box_get_active_iter (combo, &iter))
    {
      ...
      self->blocked = TRUE;
      WIKIPAD_SETTING_SET_ENUM (TOOLBAR_ICON_SIZE, icon_size);
      self->blocked = FALSE;
    }
}
...
/* update the combo when the setting changes */
static void
wikipad_prefs_dialog_toolbar_icon_size_setting_changed (WikipadPrefsDialog *self,
                                                         gchar               *key,
                                                         GSettings           *settings)
{
  GtkComboBox  *combo;
  ...
  icon_size = WIKIPAD_SETTING_GET_ENUM (TOOLBAR_ICON_SIZE);
...
#define wikipad_builder_get_widget(builder, name) \
  GTK_WIDGET (gtk_builder_get_object (builder, name))



static void
wikipad_prefs_dialog_init (WikipadPrefsDialog *self)
{
  GError *error = NULL;
  GtkWidget *notebook;
  ...
  if (! gtk_builder_add_from_string (self->builder,
                                     wikipad_prefs_dialog_ui,
                                     wikipad_prefs_dialog_ui_length,
                                     &error))
    {
  ...
  notebook = wikipad_builder_get_widget (self->builder, WID_NOTEBOOK);
  ...
  check = wikipad_builder_get_widget (self->builder, WID_SHOW_RIGHT_MARGIN_CHECK);
  widget = wikipad_builder_get_widget (self->builder, WID_RIGHT_MARGIN_SPIN);
  ...
  check = wikipad_builder_get_widget (self->builder, WID_USE_DEFAULT_FONT_CHECK);
  widget = wikipad_builder_get_widget (self->builder, WID_FONT_BUTTON);
  ...
  check = wikipad_builder_get_widget (self->builder, WID_TOOLBAR_VISIBLE_CHECK);

  widget = wikipad_builder_get_widget (self->builder, WID_TOOLBAR_STYLE_LABEL);
  g_object_bind_property (check, "active", widget, "sensitive", G_BINDING_SYNC_CREATE);
  widget = wikipad_builder_get_widget (self->builder, WID_TOOLBAR_STYLE_COMBO);
  g_object_bind_property (check, "active", widget, "sensitive", G_BINDING_SYNC_CREATE);
  gtk_combo_box_set_active (GTK_COMBO_BOX (widget), WIKIPAD_SETTING_GET_ENUM (TOOLBAR_STYLE));

  widget = wikipad_builder_get_widget (self->builder, WID_TOOLBAR_ICON_SIZE_LABEL);
  g_object_bind_property (check, "active", widget, "sensitive", G_BINDING_SYNC_CREATE);
  widget = wikipad_builder_get_widget (self->builder, WID_TOOLBAR_ICON_SIZE_COMBO);
  g_object_bind_property (check, "active", widget, "sensitive", G_BINDING_SYNC_CREATE);
  wikipad_prefs_dialog_toolbar_icon_size_setting_changed (self, NULL, NULL);

  /* bind checkboxes to settings */
#define BIND_CHECKBOX(setting)                                           \
  WIKIPAD_SETTING_BIND (setting,                                        \
...
#undef BIND_CHECKBOX
  ...
  g_signal_connect_swapped (gtk_builder_get_object (self->builder, WID_TAB_MODE_COMBO),
                            "changed",
                            G_CALLBACK (wikipad_prefs_dialog_tab_mode_changed),
                            self);

  /* update tab mode combo when setting changes */
  WIKIPAD_SETTING_CONNECT_OBJECT (INSERT_SPACES,
                                   G_CALLBACK (wikipad_prefs_dialog_tab_mode_setting_changed),
                                   self,
                                   G_CONNECT_SWAPPED);
  ...
  g_signal_connect_swapped (gtk_builder_get_object (self->builder, WID_SMART_HOME_END_COMBO),
                            "changed",
                            G_CALLBACK (wikipad_prefs_dialog_home_end_changed),
                            self);

  /* update home/end combo when setting changes */
  WIKIPAD_SETTING_CONNECT_OBJECT (SMART_HOME_END,
                                   G_CALLBACK (wikipad_prefs_dialog_home_end_setting_changed),
                                   self,
                                   G_CONNECT_SWAPPED);
  ...
  g_signal_connect_swapped (gtk_builder_get_object (self->builder, WID_TOOLBAR_STYLE_COMBO),
                            "changed",
                            G_CALLBACK (wikipad_prefs_dialog_toolbar_style_changed),
                            self);

  /* update toolbar style combo when the setting changes */
  WIKIPAD_SETTING_CONNECT_OBJECT (TOOLBAR_STYLE,
                                   G_CALLBACK (wikipad_prefs_dialog_toolbar_style_setting_changed),
                                   self,
                                   G_CONNECT_SWAPPED);
  ...
  g_signal_connect_swapped (gtk_builder_get_object (self->builder, WID_TOOLBAR_ICON_SIZE_COMBO),
                            "changed",
                            G_CALLBACK (wikipad_prefs_dialog_toolbar_icon_size_changed),
                            self);

  /* update toolbar icon size combo when setting changes */
  WIKIPAD_SETTING_CONNECT_OBJECT (TOOLBAR_ICON_SIZE,
                                   G_CALLBACK (wikipad_prefs_dialog_toolbar_icon_size_setting_changed),
                                   self,
                                   G_CONNECT_SWAPPED);
}
...
GtkWidget *
wikipad_prefs_dialog_new (void)
{
  return g_object_new (WIKIPAD_TYPE_PREFS_DIALOG, NULL);
}

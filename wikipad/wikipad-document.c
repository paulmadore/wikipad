...
#include <wikipad/wikipad-close-button.h>
...
#define WIKIPAD_DOCUMENT_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), WIKIPAD_TYPE_DOCUMENT, WikipadDocumentPrivate))



static void      wikipad_document_finalize                (GObject                *object);
static void      wikipad_document_notify_cursor_position  (GtkTextBuffer          *buffer,
                                                            GParamSpec             *pspec,
                                                            WikipadDocument       *document);
static void      wikipad_document_notify_has_selection    (GtkTextBuffer          *buffer,
                                                            GParamSpec             *pspec,
                                                            WikipadDocument       *document);
static void      wikipad_document_notify_overwrite        (GtkTextView            *textview,
                                                            GParamSpec             *pspec,
                                                            WikipadDocument       *document);
static void      wikipad_document_notify_language         (GtkSourceBuffer        *buffer,
                                                            GParamSpec             *pspec,
                                                            WikipadDocument       *document);
static void      wikipad_document_drag_data_received      (GtkWidget              *widget,
                                                            ...
                                                            guint                   drag_time,
                                                            WikipadDocument       *document);
static void      wikipad_document_filename_changed        (WikipadDocument       *document,
                                                            const gchar            *filename);
static void      wikipad_document_label_color             (WikipadDocument       *document);
static void      wikipad_document_tab_button_clicked      (GtkWidget              *widget,
                                                            WikipadDocument       *document);
...
struct _WikipadDocumentClass
{
  GtkScrolledWindowClass __parent__;
};

struct _WikipadDocumentPrivate
{
...
WikipadDocument *
wikipad_document_new (void)
{
  return g_object_new (WIKIPAD_TYPE_DOCUMENT, NULL);
}



G_DEFINE_TYPE (WikipadDocument, wikipad_document, GTK_TYPE_SCROLLED_WINDOW)



static void
wikipad_document_class_init (WikipadDocumentClass *klass)
{
  GObjectClass *gobject_class;

  g_type_class_add_private (klass, sizeof (WikipadDocumentPrivate));

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = wikipad_document_finalize;
  ...
  document_signals[CURSOR_CHANGED] =
    g_signal_new (I_("cursor-changed"),
                  ...
                  G_SIGNAL_RUN_LAST,
                  0, NULL, NULL,
                  _wikipad_marshal_VOID__INT_INT_INT,
...
static void
wikipad_document_finalize (GObject *object)
{
  WikipadDocument *document = WIKIPAD_DOCUMENT (object);
  ...
  (*G_OBJECT_CLASS (wikipad_document_parent_class)->finalize) (object);
}
...
static void
wikipad_document_notify_language (GtkSourceBuffer  *buffer,
                                   GParamSpec       *pspec,
                                   WikipadDocument *document)
{
...
static void
wikipad_document_drag_data_received (GtkWidget        *widget,
                                      GdkDragContext   *context,
                                      gint              x,
                                      gint              y,
                                      GtkSelectionData *selection_data,
...
static void
wikipad_document_filename_changed (WikipadDocument *document,
                                    const gchar      *filename)
{
      ...
      if (G_UNLIKELY (document->priv->label))
        {
          ...
          /* update label color */
          wikipad_document_label_color (document);
        }
    }
}
...
void
wikipad_document_set_overwrite (WikipadDocument *document,
                                 gboolean          overwrite)
{
  g_return_if_fail (WIKIPAD_IS_DOCUMENT (document));
...
void
wikipad_document_focus_textview (WikipadDocument *document)
{
  g_return_if_fail (WIKIPAD_IS_DOCUMENT (document));
...
void
wikipad_document_send_signals (WikipadDocument *document)
{
  g_return_if_fail (WIKIPAD_IS_DOCUMENT (document));

  /* re-send the cursor changed signal */
  wikipad_document_notify_cursor_position (document->buffer, NULL, document);

  /* re-send the overwrite signal */
  wikipad_document_notify_overwrite (GTK_TEXT_VIEW (document->textview), NULL, document);

  /* re-send the selection status */
  wikipad_document_notify_has_selection (document->buffer, NULL, document);

  /* re-send the language signal */
  wikipad_document_notify_language (GTK_SOURCE_BUFFER (document->buffer), NULL, document);
}



GtkWidget *
wikipad_document_get_tab_label (WikipadDocument *document)
{
  ...
  /* create the label */
  document->priv->label = gtk_label_new (wikipad_document_get_basename (document));
  ...
  button = wikipad_close_button_new ();
  ...
  g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (wikipad_document_tab_button_clicked), document);
...
static void
wikipad_document_tab_button_clicked (GtkWidget        *widget,
                                      WikipadDocument *document)
{
...
const gchar *
wikipad_document_get_basename (WikipadDocument *document)
{
  static gint untitled_counter = 0;

  g_return_val_if_fail (WIKIPAD_IS_DOCUMENT (document), NULL);
...
const gchar *
wikipad_document_get_filename (WikipadDocument *document)
{
  g_return_val_if_fail (WIKIPAD_IS_DOCUMENT (document), NULL);

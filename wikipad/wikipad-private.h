...
#ifndef __WIKIPAD_PRIVATE_H__
#define __WIKIPAD_PRIVATE_H__
...
#define WIKIPAD_UNSET_FLAG(flags,flag) G_STMT_START{ ((flags) &= ~(flag)); }G_STMT_END
#define WIKIPAD_HAS_FLAG(flags,flag)   (((flags) & (flag)) != 0)
...
#define WIKIPAD_PARAM_READWRITE (G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB)

/* support for canonical strings and quarks */
#define I_(string)  (g_intern_static_string (string))

/* convienient function for setting object data */
#define wikipad_object_set_data(object,key,data)              (g_object_set_qdata ((object), \
                                                                g_quark_from_static_string (key), (data)))
#define wikipad_object_set_data_full(object,key,data,destroy) (g_object_set_qdata_full ((object), \
                                                                g_quark_from_static_string (key), (data), (GDestroyNotify) (destroy)))
#define wikipad_object_get_data(object,key)                   (g_object_get_qdata ((object), g_quark_try_string (key)))
...
static inline guint
wikipad_disconnect_by_func_ (gpointer  instance,
                              GCallback callback,
                              gpointer  data)
{
  union {
...
#define wikipad_disconnect_by_func(instance, callback, data) \
  wikipad_disconnect_by_func_ (instance, G_CALLBACK (callback), data)

G_END_DECLS

#endif /* !__WIKIPAD_PRIVATE_H__ */

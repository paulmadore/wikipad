...
#define WIKIPAD_VIEW_DEFAULT_FONT "Monospace"
#define wikipad_view_get_buffer(view) (gtk_text_view_get_buffer (GTK_TEXT_VIEW (view)))



static void      wikipad_view_finalize                      (GObject            *object);
static void      wikipad_view_set_property                  (GObject            *object,
                                                              guint               prop_id,
                                                              const GValue       *value,
                                                              GParamSpec         *pspec);
static void      wikipad_view_get_property                  (GObject            *object,
                                                              guint               prop_id,
                                                              GValue             *value,
                                                              GParamSpec         *pspec);
#ifdef HAVE_MULTISELECT
static void      wikipad_view_style_set                     (GtkWidget          *widget,
                                                              GtkStyle           *previous_style);
static gint      wikipad_view_expose                        (GtkWidget          *widget,
                                                              GdkEventExpose     *event);
#endif
static gboolean  wikipad_view_key_press_event               (GtkWidget          *widget,
                                                              GdkEventKey        *event);
#ifdef HAVE_MULTISELECT
static gboolean  wikipad_view_button_press_event            (GtkWidget          *widget,
                                                              GdkEventButton     *event);
static gboolean  wikipad_view_button_release_event          (GtkWidget          *widget,
                                                              GdkEventButton     *event);
...
static void      wikipad_view_transpose_lines               (GtkTextBuffer       *buffer,
                                                              GtkTextIter         *start_iter,
                                                              GtkTextIter         *end_iter);
...
wikipad_view_class_init (WikipadViewClass *klass)
{
  ...
  gobject_class->finalize = wikipad_view_finalize;
  gobject_class->set_property = wikipad_view_set_property;
  gobject_class->get_property = wikipad_view_get_property;

  widget_class = GTK_WIDGET_CLASS (klass);
  widget_class->key_press_event      = wikipad_view_key_press_event;
#ifdef HAVE_MULTISELECT
  widget_class->expose_event         = wikipad_view_expose;
  widget_class->style_set            = wikipad_view_style_set;
  widget_class->button_press_event   = wikipad_view_button_press_event;
  widget_class->button_release_event = wikipad_view_button_release_event;
#endif
...
wikipad_view_init (WikipadView *view)
{
  ...
  g_signal_connect (view,
                    "notify::buffer",
                    G_CALLBACK (wikipad_view_buffer_changed),
                    NULL);
  ...
  g_signal_connect (GTK_TEXT_VIEW (view)->im_context, "commit",
                    G_CALLBACK (wikipad_view_commit_handler), view);
#endif

  /* bind Gsettings */
#define BIND_(setting, prop) \
  WIKIPAD_SETTING_BIND (setting, view, prop, G_SETTINGS_BIND_DEFAULT)
  ...
  WIKIPAD_SETTING_BIND (COLOR_SCHEME, view, "color-scheme",
                         G_SETTINGS_BIND_DEFAULT |
                           G_SETTINGS_BIND_GET_NO_CHANGES);

  /* override with default font when the setting is enabled */
  WIKIPAD_SETTING_CONNECT_OBJECT (USE_DEFAULT_FONT,
                                   G_CALLBACK (wikipad_view_use_default_font_setting_changed),
                                   view,
                                   G_CONNECT_SWAPPED);
...
wikipad_view_finalize (GObject *object)
{
  ...
  (*G_OBJECT_CLASS (wikipad_view_parent_class)->finalize) (object);
}



static void
wikipad_view_set_property (GObject      *object,
                            guint         prop_id,
                            const GValue *value,
                            GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_FONT_NAME:
      wikipad_view_set_font_name (view, g_value_get_string (value));
      break;
    case PROP_SHOW_WHITESPACE:
      wikipad_view_set_show_whitespace (view, g_value_get_boolean (value));
      break;
    case PROP_SHOW_LINE_ENDINGS:
      wikipad_view_set_show_line_endings (view, g_value_get_boolean (value));
      break;
    case PROP_COLOR_SCHEME:
      wikipad_view_set_color_scheme (view, g_value_get_string (value));
      break;
    case PROP_WORD_WRAP:
      wikipad_view_set_word_wrap (view, g_value_get_boolean (value));
      break;
    case PROP_MATCH_BRACES:
      wikipad_view_set_match_braces (view, g_value_get_boolean (value));
      break;
    default:
...
static void
wikipad_view_get_property (GObject    *object,
                            guint       prop_id,
                            GValue     *value,
                            GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_FONT_NAME:
      g_value_set_string (value, wikipad_view_get_font_name (view));
      break;
    case PROP_SHOW_WHITESPACE:
      g_value_set_boolean (value, wikipad_view_get_show_whitespace (view));
      break;
    case PROP_SHOW_LINE_ENDINGS:
      g_value_set_boolean (value, wikipad_view_get_show_line_endings (view));
      break;
    case PROP_COLOR_SCHEME:
      g_value_set_string (value, wikipad_view_get_color_scheme (view));
      break;
    case PROP_WORD_WRAP:
      g_value_set_boolean (value, wikipad_view_get_word_wrap (view));
      break;
    case PROP_MATCH_BRACES:
      g_value_set_boolean (value, wikipad_view_get_match_braces (view));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
#ifdef HAVE_MULTISELECT
static gboolean
wikipad_view_expose (GtkWidget      *widget,
                      GdkEventExpose *event)
{
  ...
  if (G_UNLIKELY (view->selection_length == -1
      ...
      && event->window == gtk_text_view_get_window (textview, GTK_TEXT_WINDOW_TEXT)))
    {
      ...
      wikipad_view_selection_draw (view, FALSE);
    }

  /* gtk can draw the text now */
  return (*GTK_WIDGET_CLASS (wikipad_view_parent_class)->expose_event) (widget, event);
}



static void
wikipad_view_style_set (GtkWidget *widget,
                         GtkStyle  *previous_style)
{
  GtkStyle      *style;
  ...
  /* run widget handler */
  (*GTK_WIDGET_CLASS (wikipad_view_parent_class)->style_set) (widget, previous_style);
  ...
  if (previous_style)
    {
      ...
      /* get the text buffer */
      buffer = wikipad_view_get_buffer (view);
                               ...
                               GdkEventKey *event)
{
  ...
  switch (event->keyval)
    {
      ...
      case GDK_KP_Home:
        ...
        if (gtk_text_iter_starts_line (&iter)
            && wikipad_util_forward_iter_to_text (&iter, NULL))
          {
             ...
             wikipad_view_scroll_to_cursor (view);
        ...
        if (view->selection_marks != NULL && is_editable)
          {
            ...
            if (view->selection_length > 0)
              wikipad_view_delete_selection (view);
#ifdef HAVE_MULTISELECT
            else
              wikipad_view_selection_destroy (view);
#endif
            return TRUE;
          }
        break;
        ...
        if (view->selection_marks != NULL && is_editable)
          {
            ...
            wikipad_view_selection_key_press_event (view, NULL, GDK_BackSpace, modifiers);
  ...
  return (*GTK_WIDGET_CLASS (wikipad_view_parent_class)->key_press_event) (widget, event);
}
  ...
  if (G_UNLIKELY (view->selection_marks != NULL))
    {
      ...
      wikipad_view_selection_key_press_event (view, str, 0, 0);
    }
}
                                  ...
                                  GdkEventButton *event)
{
  ...
  if (event->state & GDK_CONTROL_MASK
      ...
      && event->type == GDK_BUTTON_PRESS)
    {
      ...
      view->selection_timeout_id = g_timeout_add_full (G_PRIORITY_LOW, 50, wikipad_view_selection_timeout,
                                                       view, wikipad_view_selection_timeout_destroy);
  ...
  else if (event->type == GDK_2BUTTON_PRESS && event->button == 1)
    {
      ...
      if (wikipad_view_selection_word_range (&iter, &start_iter, &end_iter))
        {
          /* get the buffer */
          buffer = wikipad_view_get_buffer (view);
  ...
  return (*GTK_WIDGET_CLASS (wikipad_view_parent_class)->button_press_event) (widget, event);
}
                                    ...
                                    GdkEventButton *event)
{
  ...
  if (G_UNLIKELY (view->selection_timeout_id != 0))
    {
      ...
      wikipad_view_selection_draw (view, TRUE);
  ...
  return (*GTK_WIDGET_CLASS (wikipad_view_parent_class)->button_release_event) (widget, event);
}
#endif
...
#ifdef HAVE_MULTISELECT
static gboolean
wikipad_view_selection_word_range (const GtkTextIter *iter,
                                    GtkTextIter       *range_start,
                                    GtkTextIter       *range_end)
{
  /* init iters */
  *range_start = *range_end = *iter;
  ...
  if (!wikipad_util_iter_inside_word (iter))
    {
      /* walk back and forward to select as much spaces as possible */
      if (!wikipad_util_iter_forward_text_start (range_end))
        return FALSE;

      if (!wikipad_util_iter_backward_text_start (range_start))
        return FALSE;
    }
  else
    {
      /* return false when we could not find a word start */
      if (!wikipad_util_iter_backward_word_start (range_start))
        return FALSE;

      /* return false when we could not find a word end */
      if (!wikipad_util_iter_forward_word_end (range_end))
        return FALSE;
    }
                                         ...
                                         guint         modifiers)
{
  ...
  if (view->selection_editing == FALSE && view->selection_length > 0)
    wikipad_view_selection_delete_content (view);
  ...
  for (li = view->selection_marks; li != NULL; li = li->next)
    {
      ...
      if (keyval == GDK_Tab)
        {
          /* insert a (soft) tab */
          wikipad_view_indent_increase (view, &end_iter);
        }
...
static void
wikipad_view_selection_timeout_destroy (gpointer user_data)
{
  WIKIPAD_VIEW (user_data)->selection_timeout_id = 0;
}
                               ...
                               GtkTextIter  *iter)
{
  ...
  if (gtk_source_view_get_insert_spaces_instead_of_tabs (GTK_SOURCE_VIEW (view)))
    {
      /* get the offset */
      offset = wikipad_util_get_real_line_offset (iter, tab_size);
          ...
          for (i = start_line; i <= end_line && i < G_MAXINT; i++)
            {
              ...
              if (increase)
                wikipad_view_indent_increase (view, &start_iter);
              else
                wikipad_view_indent_decrease (view, &start_iter);
            }
        }
...
static void
wikipad_view_transpose_range (GtkTextBuffer *buffer,
                               GtkTextIter   *start_iter,
                               GtkTextIter   *end_iter)
{
...
static void
wikipad_view_transpose_lines (GtkTextBuffer *buffer,
                               GtkTextIter   *start_iter,
                               GtkTextIter   *end_iter)
{
  GString *string;
...
static void
wikipad_view_transpose_words (GtkTextBuffer *buffer,
                               GtkTextIter   *iter)
{
  ...
  /* left word end */
  end_left = *iter;
  if (!wikipad_util_iter_backward_text_start (&end_left))
    return;

  /* left word start */
  start_left = end_left;
  if (!wikipad_util_iter_backward_word_start (&start_left))
    return;

  /* right word start */
  start_right = *iter;
  if (!wikipad_util_iter_forward_text_start (&start_right))
    return;

  /* right word end */
  end_right = start_right;
  if (!wikipad_util_iter_forward_word_end (&end_right))
    return;
  ...
  if (view->selection_marks != NULL)
    {
      ...
      wikipad_view_transpose_multi_selection (buffer, view);
    }
  else
#endif
  if (gtk_text_buffer_get_selection_bounds (buffer, &sel_start, &sel_end))
    {
      ...
      if (gtk_text_iter_get_line (&sel_start) == gtk_text_iter_get_line (&sel_end))
        {
          /* reverse selection */
          wikipad_view_transpose_range (buffer, &sel_start, &sel_end);
        }
      else
        {
          /* reverse lines */
          wikipad_view_transpose_lines (buffer, &sel_start, &sel_end);
        }
  ...
  else
    {
      ...
      if (gtk_text_iter_starts_line (&sel_start))
      ...
      else if (wikipad_util_iter_inside_word (&sel_start))
        {
      ...
      else
        {
          ...
          wikipad_view_transpose_words (buffer, &sel_start);
        }
    }
...
wikipad_view_clipboard_cut (WikipadView *view)
{
  ...
  g_return_if_fail (WIKIPAD_IS_VIEW (view));
  g_return_if_fail (wikipad_view_get_selection_length (view, NULL) > 0);
  ...
  if (view->selection_marks != NULL)
    {
      ...
      string = wikipad_view_selection_string (view);
          ...
          switch (type)
            {
              ...
              case TITLECASE:
                converted = wikipad_util_utf8_strcapital (text);
                break;

              case OPPOSITE_CASE:
                converted = wikipad_util_utf8_stropposite (text);
                break;
  ...
  for (;;)
    {
      ...
      if (type == SPACES_TO_TABS)
      ...
      else if (type == TABS_TO_SPACES && c == '\t')
        {
          ...
          offset = wikipad_util_get_real_line_offset (&start_iter, tab_size);
                      ...
                      gint          type)
{
  ...
  wikipad_view_indent_selection (view, type == INCREASE_INDENT, TRUE);
}
                             ...
                             const gchar  *font_name)
{
  ...
  if (font_name == NULL)
    font_name = WIKIPAD_VIEW_DEFAULT_FONT;
...
wikipad_view_update_font (WikipadView *view)
{
  ...
  use_default = WIKIPAD_SETTING_GET_BOOLEAN (USE_DEFAULT_FONT);

  if (use_default)
    {
      PangoFontDescription *font_desc;
      font_desc = pango_font_description_from_string (WIKIPAD_VIEW_DEFAULT_FONT);
      wikipad_view_override_font (view, font_desc);
      pango_font_description_free (font_desc);
    }
  else
    wikipad_view_override_font (view, view->font_desc);
}
                                   ...
                                   gboolean      show)
{
  ...
  wikipad_view_update_draw_spaces (view);
  ...
  if (g_strcmp0 (color_scheme, view->color_scheme) != 0)
    {
      ...
      wikipad_view_buffer_changed (view, NULL, NULL);

...
static void           wikipad_print_finalize              (GObject                 *object);
static void           wikipad_print_settings_load         (GtkPrintOperation       *operation);
static void           wikipad_print_settings_save_foreach (const gchar             *key,
                                                            const gchar             *value,
                                                            gpointer                 user_data);
static void           wikipad_print_settings_save         (GtkPrintOperation       *operation);
static void           wikipad_print_begin_print           (GtkPrintOperation       *operation,
                                                            GtkPrintContext         *context);
static void           wikipad_print_draw_page             (GtkPrintOperation       *operation,
                                                            GtkPrintContext         *context,
                                                            gint                     page_nr);
...
static void           wikipad_print_done                  (GtkPrintOperation       *operation,
...
wikipad_print_class_init (MousepadPrintClass *klass)
{
  ...
  gobject_class->finalize = wikipad_print_finalize;

  gtkprintoperation_class = GTK_PRINT_OPERATION_CLASS (klass);
  gtkprintoperation_class->begin_print = wikipad_print_begin_print;
  gtkprintoperation_class->draw_page = wikipad_print_draw_page;
  gtkprintoperation_class->create_custom_widget = wikipad_print_create_custom_widget;
  gtkprintoperation_class->status_changed = wikipad_print_status_changed;
  gtkprintoperation_class->done = wikipad_print_done;
}
...
wikipad_print_finalize (GObject *object)
{
  ...
  (*G_OBJECT_CLASS (wikipad_print_parent_class)->finalize) (object);
}
...
wikipad_print_settings_load (GtkPrintOperation *operation)
{
  ...
  g_return_if_fail (WIKIPAD_IS_DOCUMENT (print->document));
  g_return_if_fail (GTK_IS_WIDGET (print->document->textview));

  /* get the config file filename */
  filename = wikipad_util_get_save_location (WIKIPAD_RC_RELPATH, FALSE);
          ...
          for (i = 0; keys[i] != NULL; i++)
            {
              ...
              if (G_LIKELY (value))
                {
                  key = wikipad_util_key_name (keys[i]);
...
static void
wikipad_print_settings_save_foreach (const gchar *key,
                                      const gchar *value,
                                      gpointer     user_data)
{
  ...
  if (G_LIKELY (key && value))
    {
      config = wikipad_util_config_name (key);
...
wikipad_print_settings_save (GtkPrintOperation *operation)
{
  ...
  filename = wikipad_util_get_save_location (WIKIPAD_RC_RELPATH, TRUE);
      ...
      if (G_LIKELY (settings != NULL))
        {
          ...
          gtk_print_settings_foreach (settings, wikipad_print_settings_save_foreach, keyfile);

          /* save the contents */
          wikipad_util_save_key_file (keyfile, filename);
        }
    }
                            ...
                            GtkPrintContext   *context)
{
  ...
  if (gtk_source_print_compositor_get_print_header (print->compositor))
    {
      if (wikipad_document_get_filename (document))
        file_name = wikipad_document_get_filename (document);
      else
        file_name = wikipad_document_get_basename (document);
...
static void
wikipad_print_draw_page (GtkPrintOperation *operation,
                          GtkPrintContext   *context,
                          gint               page_nr)
{
  MousepadPrint *print = WIKIPAD_PRINT (operation);
...
static void
wikipad_print_page_setup_dialog (GtkWidget         *button,
                                  GtkPrintOperation *operation)
{
  GtkWidget        *toplevel;
  GtkPrintSettings *settings;
...
wikipad_print_attr_list_bold (void)
{
...
wikipad_print_create_custom_widget (GtkPrintOperation *operation)
{
  ...
  gtk_label_set_attributes (GTK_LABEL (label), wikipad_print_attr_list_bold ());
  ...
  button = wikipad_util_image_button (GTK_STOCK_PROPERTIES, _("_Adjust page size and orientation"));
  g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (wikipad_print_page_setup_dialog), operation);
  ...
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (button),
                                gtk_source_print_compositor_get_print_header (print->compositor));
  g_signal_connect (G_OBJECT (button), "toggled", G_CALLBACK (wikipad_print_button_toggled), print);
  ...
  g_signal_connect (G_OBJECT (print->widget_line_numbers_spin),
                    "value-changed",
                    G_CALLBACK (wikipad_print_spin_value_changed),
                    print);
...
# pragma GCC diagnostic ignored "-Wdeprecated-declarations"
  ...
  g_signal_connect (G_OBJECT (print->widget_header_font), "font-set", G_CALLBACK (wikipad_print_button_font_set), print);
  ...
  g_signal_connect (G_OBJECT (print->widget_body_font), "font-set", G_CALLBACK (wikipad_print_button_font_set), print);
  ...
  g_signal_connect (G_OBJECT (print->widget_line_numbers_font), "font-set", G_CALLBACK (wikipad_print_button_font_set), print);
...
static void
wikipad_print_status_changed (GtkPrintOperation *operation)
{
...
static void
wikipad_print_done (GtkPrintOperation       *operation,
                     GtkPrintOperationResult  result)
{
  ...
  if (result == GTK_PRINT_OPERATION_RESULT_APPLY)
    {
      /* save the settings */
      wikipad_print_settings_save (operation);
    }
}
                                     ...
                                     GError           **error)
{
  ...
  g_return_val_if_fail (WIKIPAD_IS_PRINT (print), FALSE);
  ...
  /* load settings */
  wikipad_print_settings_load (GTK_PRINT_OPERATION (print));
